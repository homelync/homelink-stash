    ...
    try {
        for (const p of configuration.plugins) {
            try {
                ...
                Logger.info(`Initialising plugin ${p.name}`);
                await plugin.init(DependencyInjectionContainer, configuration, TYPES);
                Logger.info(`Plugin ${p.name} successfully initialised`);
